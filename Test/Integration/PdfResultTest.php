...
class PdfResultTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testRenderPdfResponse()
    {
        ...
        $this->assertResponseHeader($response, 'Content-length', $this->greaterThan(1000),
            'Content-length header should be at least 1K');
        $this->assertResponseHeader($response, 'Cache-Control',
            $this->equalTo('must-revalidate, post-check=0, pre-check=0'));
        ...
        $this->assertResponseHeader($response, 'Content-Disposition',
            $this->equalTo('attachment; filename="testpdf.pdf"'));
     ...
     */
    private function assertResponseHeader(
